import React from "react";

const Hero = () => {
  return (
    <section className="w-full" style={{ backgroundColor: '#F1EEEA' }}>
      <div className="w-full px-4 md:px-10 pt-20 pb-16 flex flex-col items-center">
        <h1 className="text-5xl md:text-6xl font-light text-[#1a1a1a] mb-10 text-center">
          Client Impact
        </h1>
        <p className="max-w-3xl text-lg md:text-xl leading-relaxed text-[#1a1a1a] text-center">
          Discover how we help our clients achieve transformational results through strategic
          consulting and innovative solutions.
        </p>
      </div>
      <div className="w-full px-4 md:px-10 pb-0 flex justify-center">
        <div className="w-full overflow-hidden rounded-3xl">
          <div className="h-[420px] md:h-[500px] w-full">
            <img
              src="https://images.pexels.com/photos/3182834/pexels-photo-3182834.jpeg"
              alt="Client Impact"
              className="w-full h-full object-cover object-center"
            />
          </div>
        </div>
      </div>
      <div className="w-full py-20">
        <div className="max-w-7xl mx-auto px-4 md:px-10">
          <p className="text-sm md:text-base leading-relaxed text-[#1a1a1a] text-center max-w-4xl mx-auto">
            From Fortune 500 transformations to innovative start-up scale-ups, we partner with
            organizations worldwide to deliver measurable results. Our client success stories
            showcase the tangible value we create through strategic insights, operational excellence,
            and cutting-edge technology solutions.
          </p>
        </div>
      </div>
    </section>
  );
};

export default Hero;
